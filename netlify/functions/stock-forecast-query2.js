...
exports.handler = async function(event) {
  ...
  try {
    ...
    const [confirmedDemandLines, quotedSoLines, quotedSoHeaders, crmLeads, invoiceHeaders, invoiceLines] = await Promise.all([
      odooCall(uid,'sale.order.line','search_read',
        [[['order_id.state','=','sale'],['product_uom_qty','>',0],['qty_delivered','<','product_uom_qty']]],
      ...
      odooCall(uid,'account.move','search_read',
        [[['move_type','=','out_invoice'],['state','=','posted'],['invoice_date','>=',histStart]]],
      ...
      odooCall(uid,'account.move.line','search_read',
        [[['move_id.move_type','=','out_invoice'],['move_id.state','=','posted'],
          ['move_id.invoice_date','>=',threeMonthsAgo],['quantity','>',0],['product_id','!=',false]]],
    ...
    return {
      ...
      body: JSON.stringify({ confirmedDemandLines, quotedSoLines, quotedSoHeaders, crmLeads, invoiceHeaders, invoiceLines }),
    };
  } catch(err) {
