...
exports.handler = async function(event) {
  ...
  try {
    ...
    // Fetch invoices without account filter first
    ...
    // Find IDs of invoices that have rent/rates lines (491000, 710000)
    const rentRatesInvoiceIds = await odooCall(uid, 'account.move.line', 'search',
    ...
    // Get the move_ids of those lines
    const rentRatesLines = rentRatesInvoiceIds.length > 0
      ? await odooCall(uid, 'account.move.line', 'search_read',
          [[['id','in',rentRatesInvoiceIds]]],
          { fields: ['move_id'], limit: 500 })
      : [];
    const excludedMoveIds = new Set(rentRatesLines.map(l =>
      Array.isArray(l.move_id) ? l.move_id[0] : l.move_id));

    // Filter out rent/rates invoices
